...
class LogoutView(View):
    def get(self, request):
        logout(request)
        return redirect("/toilet/login/")
class LoginView(FormView):
    template_name = "CRUD/login.html"
    form_class = LoginForm
    redirect_field_name = "/toilet/client-map/" 
    success_url="/toilet/client-map/"          #-------sends to client if not admin
...
class ClientRegisterView(FormView):
    ...
    success_url = "/toilet/activation-sent/"
    def form_valid(self, form):
        email = form.cleaned_data["email"]
        pword = form.cleaned_data["password"]
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        user.save()
        login(request, user)
        return HttpResponseRedirect('/toilet/client/')
    else:
...
class Hostel_CreateView(LoginRequiredMixin,UserPassesTestMixin,FormView, SuccessMessageMixin):
    login_url = "/toilet/login/"
    form_class = Hostel_Request_Form
    template_name = "main.html"
    success_url = "/toilet/view/"
...
class Hostel_Comments:
    @login_required(login_url='/toilet/login/')
    def get(request):
...
class Client_CreateView(LoginRequiredMixin,SuccessMessageMixin, FormView,CreateView ):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            lat=request.POST['Toilet_lat']
            lon=request.POST['Toilet_long']
            ...
            # Form is not valid, access the errors
            errors = form.errors
            print(errors)
...
class Hostel_AcceptView:
    ...
    def accept(self, pk):
        ...
        Del.delete()
        return HttpResponseRedirect("/toilet/toilet-request-list/")
    @user_is_admin_required
...
class Hostel_ListView(LoginRequiredMixin,UserPassesTestMixin, ListView):
    ...
    redirect_field_name='/toilet/login/'
    def test_func(self):                                #fail test only work if returned false
        if self.request.user.is_admin:
            return self.request.user
        return False   
...
class Hostel_DeleteView(LoginRequiredMixin,SuccessMessageMixin,UserPassesTestMixin,DeleteView):
    ...
    template_name = "CRUD/hostel_list.html"
    success_message = "Deleted successfully"
    success_url = "/toilet/hostel-list/"
    def test_func(self):                                #fail test only work if returned false
        if self.request.user.is_admin:
            return self.request.user
        return False   
...
class DeleteGroup:
    ...
    def deleteComment(request):
        ...
        for e in pre:
            for h in e.hostel_comment_set.all():     
                h.delete()
        return HttpResponseRedirect("/toilet/hostel-comments/")
    @user_is_admin_required
    def deleteCommentAll(request):
        Hostel_comment.objects.all().delete()
        return HttpResponseRedirect("/toilet/comments/")
#
# def home(request):
#     if request.method == 'POST':
